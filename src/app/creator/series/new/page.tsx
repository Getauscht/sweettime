            ...
            <div className="relative">
                ...
                >
                    {selectedAuthors.length > 0
                        ...
                        : <span className="text-white/40">Select {title}</span>
                    }
                </div>
                {isOpen && (
...
export default function CreateSeriesPage() {
    ...
    const fetchInitialData = async () => {
        ...
        try {
            ...
            if (currentUserAuthorRes.ok) {
                ...
                toast(`Failed to verify author profile: ${errorMessage}`, 'error');
            }

        } catch (error) {
            console.error('Error fetching initial data:', error)
            toast('An error occurred while loading creation page data.', 'error')
        } finally {
            setLoading(false);
        }
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (formData.authorIds.length === 0) {
            toast('At least one author must be selected.', 'error')
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                toast(error.error || 'Failed to create series', 'error')
            }
        } catch (error) {
            ...
            <div className="space-y-6 max-w-4xl mx-auto">
                <Card className="bg-[#0f0b14] border-white/10 p-8">
                    <div className="text-center py-12">
                        <h2 className="text-2xl font-bold text-white mb-4">No author profile found</h2>
                        <p className="text-white/60 mb-4">You need to set your profile name in account settings before you can create a series.</p>
                        <div className="flex justify-center gap-4">
                            <Link href="/profile">
                                <Button variant="default" className="bg-purple-600 hover:bg-purple-700 text-white">Go to Account Settings</Button>
                            </Link>
                            <Button variant="outline" onClick={fetchInitialData} className="border-white/10 text-white hover:bg-white/5">Retry Verification</Button>
                        </div>
                    </div>
                </Card>
            </div>
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto">
            ...
            <div>
                ...
                <p className="text-white/60 mt-2">Let's get your story out there. Fill in the details below to create your new webtoon series.</p>
            </div>
            ...
            <Card className="bg-[#0f0b14] border-white/10 p-8">
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="space-y-2">
                        ...
                        <Input id="title" placeholder="e.g., The Crimson Knight" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} required className="bg-white/5 border-white/10 text-white placeholder:text-white/40" />
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea id="description" placeholder="Provide a brief, enticing overview of your series." value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={4} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder:text-white/40 focus:outline-none focus:border-purple-500 resize-none" />
                    </div>

                    <MultiSelectAuthors title="Authors" allAuthors={allAuthors} selectedIds={formData.authorIds} onToggle={toggleAuthor} />

                    <MultiSelectAuthors title="Artists" allAuthors={allAuthors} selectedIds={formData.artistIds} onToggle={toggleArtist} />
                        ...
                        <div className="border-2 border-dashed border-white/10 rounded-lg p-8">
                            {formData.coverImage ? (
                                <div className="relative">
                                    ...
                                    <Button type="button" variant="ghost" onClick={() => setFormData({ ...formData, coverImage: '' })} className="mt-4 text-white hover:bg-white/10 mx-auto block">Remove Image</Button>
                                </div>
                            ) : (
                                <div className="text-center">
                                    <Upload className="h-12 w-12 text-white/40 mx-auto mb-4" />
                                    <label className="cursor-pointer">
                                        <span className="text-purple-400 hover:text-purple-300">{uploading ? 'Uploading...' : 'Upload a file'}</span>
                    ...
                    <div className="flex items-center gap-4 justify-end pt-4">
                        <Button type="button" variant="ghost" onClick={() => router.back()} className="text-white hover:bg-white/10">Save Draft</Button>
                        <Button type="submit" disabled={loading || uploading} className="bg-purple-600 hover:bg-purple-700 text-white">{loading ? 'Creating...' : 'Submit Series'}</Button>
                    </div>
                </form>
            </Card>
            <ToastContainer />
